...
//  if (!LMP91000_I2CWriteAndConfirmReg(LMP91000_I2C_Address, TI_LMP91000_TIACN_REG, (SENS_FEEDBACK_GAIN | SENS_RLOAD)))
//  {
//     lmp91kOK=0;
//  }
//     
...
//                                      (SENS_REF_SOURCE | SENS_INT_Z_REF_DIVIDER | SENS_BIAS_VOLTAGE | SENS_BIAS_SIGN)))
  ...
  // Set internal/external ref, internal zero setting, 0 bias level, and negative bias sign
  if (!LMP91000_I2CWriteAndConfirmReg(LMP91000_I2C_Address, TI_LMP91000_REFCN_REG, (ref_source | internal_zero)))
  {
...
{
//  if (!LMP91000_I2CSetMode(LMP91000_I2C_Address, SENS_OPERATIONAL_MODE | SENS_FET_SHORT)) // Added to prevent enabling potentiostat when in OCP mode
    if (!LMP91000_I2CSetMode(LMP91000_I2C_Address, TI_LMP91000_TIATEMP_MEASURE_MODE)) // Commented out to prevent enabling potentiostat when in OCP mode
  {
    lmp91kOK = 0;
  }
                                       ...
                                       unsigned char op_mode)
{
  ...
  LMP91000_I2CConfigureForO2Sensor(tia_gain, r_load, ref_source, internal_zero); // Basically sets the REF and TIA registers
//  
  LMP91000_I2CSwitchVoutToO2Sensor(op_mode);
   // Configure LMP9100 to output temp on vout, and o2 on C2
//   LMP91000_I2CSwitchVoutToTempSensor(); // Commented out this to prevent from going into an unwated operational mode (SS 2018)
